...
In the synchronized singleton-star output of the three-spoke Boolean-cube classifier, every source has three immediate upstream consumers. Puncturing an internal source from the retained top cover and comparing with the complementary rank-two row gives the internal-puncture three-to-two entrance to first-source descent. Independently, `(A,s,C)|J_{tu}|B` is already a literal spanning three-forest of `H`, so the one-cut/two-join cap trichotomy applies directly to either nonanchor rail. Finally, inserting `s` into the displayed spectator path either gives a physical exact two-cover `J_{tu}|B_s` with transition count at most two or, if every slot fails, S9031 gives a local star-triangle/reverse-hook obstruction. In either insertion branch there is an immediate E9003 entrance: successful insertion yields a nonidentical exact cover of the same pair-deletion residue, while failed insertion literally contains a proper source trimer born in the retained phase-one packet. Hence every synchronized-star output feeds first-source phase descent without the later transition zipper or residual chase.

## S9042 — Backward E9003 Consumption Portals

Inside a retained phase-one prepayment packet, each of the following is already a complete first-source phase-descent entrance: an exact-two-cover-cardinality split matching of transition count strictly below the retained exact cover, whether physical or carrying an explicit bad-turn/backtrack/cycle defect; a singleton-defect loop prefix; and any physical spanning three-forest on a common pair- or triple-deletion residue that also carries a retained exact two-cover. In the three-forest case, a cross-component endpoint bridge either has a bad seam whose reversal is a proper trimer or produces a nonidentical exact two-cover; at most one ordered bridge can merely reconstruct the retained cover. Applied to the sharp canonical exchange staircase, these portals consume every E9005 terminal interface, including the source-rooted and `v`-rooted first-loss branches one step before their specialization.

## S9043 — Zero-Transition and Terminal-Wall Shortcuts

If split-copy matchings `F,J` satisfy `|F|=|J|+1` and `tau(J)=0`, then either some union of alternating components has cardinality excess one and transition weight strictly below `tau(F)`, or `F triangle J` has a unique `F`-heavy alternating path carrying every transition of `F`, no `J`-heavy component, and only balanced zero-weight components otherwise. In the synchronized zero-transition comparison, choosing a source incident with an `X|B` transition and orienting that unique heavy path from the transition-bearing split copy forces the first-positive zipper index to be one; this is a one-ended statement only. Separately, in any hypothetical smallest counterexample, a globally longest tight path `A` has an exact complementary two-cover `B|C`, and `A|B|C` is already a no-slide terminal wall, yielding the certified terminal-wall opposite-polarity pair and retained turn directly. The terminal-wall statement constructs the phase-zero interface but does not itself prove global extinction.
