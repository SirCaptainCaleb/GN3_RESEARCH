...
Immediately after structural synthesis and conceptual ascent, the Director/Vice Director must run a **moonshot pass**. A moonshot is a deliberately ambitious mathematical target whose probability of immediate success may be low, but whose success would close the theorem, collapse a major branch, remove a parameter or case hierarchy, or expose a substantially stronger parent theorem.

Moonshots are not predictions and are not presumed true. Their purpose is to force the search above incremental local progress and to test whether the current frontier is an artifact of working at too low an abstraction level.

A moonshot pass should:

- formulate at least one concrete closure-level or branch-collapsing statement, construction, reduction, or invariant suggested by the synthesized mathematics;
- prefer targets that would eliminate whole parameter ranges, case trees, or layers of machinery rather than merely strengthen the next local lemma;
- state the mathematical payoff explicitly: what major part of the proof would disappear or become immediate if the moonshot succeeded;
- identify the shortest plausible attack, including what existing audited or live results would serve as inputs;
- actively seek a decisive counterexample, obstruction, or necessary missing hypothesis when a moonshot resists proof, because a clean failure can reveal the correct parent theorem;
- avoid dressing ordinary next-step work up as a moonshot; genuine moonshots should reach materially beyond the current proof granularity;
- keep moonshots mathematically intrinsic. They may be bold, but they still obey the project’s language, audit, and computation disciplines.

Each Director cycle records its active moonshot or moonshots in `#gn3-guidance`. A moonshot may become the main guidance when its payoff and plausibility justify concentrating the team on it; otherwise it remains a parallel high-upside attack beside the shortest favored route. Researchers are encouraged to spend real effort on these attacks rather than treating them as decorative speculation.

Moonshot generation is downstream of synthesis and ascent: first understand the mathematics as well as possible, then ask what audacious statement would make most of the remaining complexity disappear.

### Outputs of a synthesis-and-ascent pass

A pass should leave the project with the clearest available answer to seven questions:
...
4. **What moonshot target would close the theorem or collapse the most remaining structure if it succeeded?**
5. **What is the most natural abstraction hierarchy and shortest plausible proof route now?**
6. **What exact unresolved mathematical statement should research attack next?**
...
## Astra handoff and hot-shelf preparation

Before an Astra escalation, the Vice Director prepares a **shelf-first strategic handoff**.

The sequence is:

1. **Finish ordinary shelf work first.** Any certified mathematics that is already conceptually stable enough for SHELVES/CORE/ or SHELVES/UTILITY/ should be composed, audited as required, and admitted there before Astra is briefed. Do not force settled mathematics to remain in Slack merely because an escalation is imminent.
2. **Refresh SHELVES/HOT/ASTRA.md.** Place there only recent mathematics, negative evidence, or unresolved structure that materially shapes the current strategic frontier and is not yet adequately represented in the ordinary shelves.
3. **Prune duplication.** Remove hot items that have become represented in CORE/ or UTILITY/, have been superseded, have been retracted, or no longer affect the strategic question.
4. **Establish the Astra delta.** Identify the Git commit corresponding to Astra's last trustworthy shelf synchronization, when known, and the current handoff commit after ordinary shelf updates and hot-shelf refresh.
5. **Escalate from the shelf delta.** The Astra handoff should direct Astra to the Git delta over SHELVES/CORE/, SHELVES/UTILITY/, and SHELVES/HOT/ASTRA.md between those commits, together with the concise abstraction-level question being escalated.

The hot shelf is provisional. It does not certify mathematics and is not a substitute for ordinary shelf admission. Its purpose is to make recent, strategically important mathematics available to Astra without requiring Astra to reconstruct the live state from Slack.

Astra should need Slack only as a fallback when the curated shelf delta leaves a strategically necessary ambiguity or when an exact live statement has not yet been incorporated into the hot shelf. If Astra repeatedly needs Slack to understand the current strategic state, treat that as a Vice-Director handoff failure and improve the shelf curation.
## Director cycle
...
1. **Synchronize the live picture.** Read the current proof spine, the relevant core-shelf files (normally all files touching the live proof coordinate), relevant utility-shelf files, toolkit, research tree, current guidance, relevant new research, and the certification state of mathematics the favored route actually depends on. During a long cycle, resynchronize at natural mathematical boundaries when concurrent work may have changed that picture.
...
11. **Make an explicit Astra-escalation decision and prepare the shelf handoff when needed.** Every Director cycle must decide whether the synthesized picture requires escalation to Astra. If escalation is warranted, first perform the ordinary shelf additions that are ready, refresh SHELVES/HOT/ASTRA.md, prune duplicate or settled hot material, and identify the prior Astra shelf baseline and current handoff commit when possible. Escalate when the abstraction itself needs to change: for example, a strategic reframe, stronger parent theorem that changes the theorem-level route, global bypass, major invariant or induction/minimality change, substantial proof-architecture decision, or a conflict between comparably plausible global architectures that warrants Director-level adjudication. Do not escalate merely because a local lemma is difficult, a current branch needs ordinary tactical direction, or a cleaner coordinate system refines the same already-approved parent strategy.
12. **Report the Astra decision.** The cycle report to the project owner must state either that the matter was escalated to Astra and what abstraction-level question was sent, or that it was not escalated and the concrete reason the current change remains within the existing strategic mandate. For an escalation, also report the Astra shelf baseline and handoff commits, or state that no trustworthy prior baseline exists. A silent non-escalation is not sufficient. When the case is genuinely borderline, prefer a short Astra escalation over implicitly making a major abstraction decision at Vice-Director level.
